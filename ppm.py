...
0 0 0 20 20 20 40 40 40 60 60 60 80 80 80 100 100 100 120 120 120 140 140 140'''
    ...
    def PPM_grayscale(self):
        ...
        for rowlist in newpixellist:
            col = 0
            for pixel in rowlist:
                #New R and New G and New B will be equal
                # They are all equal to 1/3 of the old R+G+B
                newpixellist[row][col][0] = (int((newpixellist[row][col][0] + newpixellist[row][col][1] + newpixellist[row][col][2]) / 3))
                newpixellist[row][col][1] = (int((newpixellist[row][col][0] + newpixellist[row][col][1] + newpixellist[row][col][2]) / 3))
                newpixellist[row][col][2] = (int((newpixellist[row][col][0] + newpixellist[row][col][1] + newpixellist[row][col][2]) / 3))
                col+=1
            row += 1
    ...
    def PPM_flip_horizontal(self):
        ...
        newpixellist = self.pixellist
        flip=copy.deepcopy(newpixellist)
        self.width = len(flip[0])
        self.height = len(flip)
        row = 0
        #Fill row by row
        for rowlist in flip:
            col = 0
            ...
            for pixel in rowlist:
                flip[row][col][0] = newpixellist[row][int(self.width)-int(1)-int(col)][0]
                flip[row][col][1] = newpixellist[row][int(self.width)-int(1)-int(col)][1]
                flip[row][col][2]= newpixellist[row][int(self.width)-int(1)-int(col)][2]
            #Flip the pixel horizontally
                col += 1
            row += 1
        print(newpixellist)
        print(self.outasciifile + " output flipped horizontally.")
        ...
        self.PPM_updatefrompixellist(flip) # This call will update all member apttributes appropriately.
    ...
    def PPM_rotateclockwise(self):
        ...
        print("w", self.width)
        print("h", self.height)
        rotate = copy.deepcopy(newpixellist)
        ...
        for i in range(self.height):
            ...
            for j in range(self.width):
                newmatrix.append(rotate[self.width - 1 - j][i])
            newpixellist.append(newmatrix)
        # Create a new matrix for the rows
        # Append the rows to the according newpixellist
        ...
        # TODO FIX ME: write the needed changes to newpixellist here
        print(self.outasciifile + " output turned clockwise 90 degrees.")

        self.PPM_updatefrompixellist(newpixellist) # This call will update all member attributes appropriately.

    # TODO FIX ME: write at least one additional PPM class method

    def PPM_contrast(self):
        """ Contrast each pixel of the photo

        :return: None
        """
        newpixellist = self.pixellist
        contrast = copy.deepcopy(newpixellist)
        row = 0
        for rowlist in contrast:
            col = 0
            for pixel in rowlist:
                #Get each R,B,G the negative of 255 and that color
                contrast[row][col][0] = 255 - int(newpixellist[row][col][0])
                contrast[row][col][1] = 255- int(newpixellist[row][col][1])
                contrast[row][col][2] = 255 - int(newpixellist[row][col][2])

                col += 1
            row += 1
        print(self.outasciifile + " output contrasted")


        self.PPM_updatefrompixellist(contrast)  # This call will update all member apttributes appropriately.
